...
import { ValidationHistoryList } from './validation/ValidationHistory';

export const BulkEmailValidator = () => {
  const [isProcessing, setIsProcessing] = useState(false);
  const { user } = useSupabaseUser();
  const { history, refreshHistory } = useValidationHistory(user);
  ...
  const handleFileAccepted = useCallback(async (file: File) => {
    ...
    try {
      ...
      if (response.data.resultPath) {
        // Fetch the result file
        const resultResponse = await axios.get(`/uploads/${response.data.resultPath}`, {
          responseType: 'blob'
        });
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
      <div className="p-6">
        ...
        <ValidationHistoryList history={history} />
      </div>
    </div>
  );
};